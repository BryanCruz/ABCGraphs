...
pub fn run_test(file_path: &String, n_executions: usize, single_target: bool) -> u128 {
    ...
    let algorithm = if single_target {
        run_algorithm_for_single_target
    } else {
        run_algorithm_for_all_targets
    };

    let execution_times_in_ms: Vec<u128> = (0..n_executions)
        .map(|i| {
            println!("Starting execution {} {}", file_path, i);
            let start_time = Instant::now();
            algorithm(&parsed_graph, &input_graph.queries);
